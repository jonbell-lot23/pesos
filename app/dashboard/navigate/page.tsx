...
export default function NavigatePage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        setLoading(true);
        // This would need to be replaced with an actual API endpoint
        const response = await fetch(`/api/posts-by-source?sourceId=${selectedSource}`);
        if (!response.ok) {
  ...
  const placeholderSources = [
    { id: 1, url: "https://example.com/feed1", lastPost: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(), isUserSource: true },
    { id: 2, url: "https://blog.example.org/rss", lastPost: new Date(Date.now() - 12 * 60 * 60 * 1000).toISOString(), isUserSource: true },
    { id: 3, url: "https://news.example.net/feed", lastPost: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), isUserSource: false },
    { id: 4, url: "https://podcast.example.com/rss", lastPost: null, isUserSource: false },
    { id: 5, url: "https://tech.example.io/feed", lastPost: new Date(Date.now() - 1 * 60 * 60 * 1000).toISOString(), isUserSource: false },
    { id: 6, url: "https://daily.example.co/rss", lastPost: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(), isUserSource: false }
  ];
  ...
  const placeholderPosts = [
    { 
      ...
      description: "<p>This is the content of the first post. It can contain <b>HTML</b> formatting.</p>",
      userId: "user123",
      isUserPost: true
    },
    { 
      ...
      description: "<p>Content of the second post with more details and information.</p><p>This has multiple paragraphs.</p>",
      userId: "user123",
      isUserPost: true
    },
    { 
      ...
      description: "<p>This is a post from another user's feed that you can see because the system shows all sources.</p>",
      userId: "user456",
      isUserPost: false
    },
    { 
      ...
      description: "<p>This is another post from a source you don't follow, showcasing the system's ability to display all content.</p>",
      userId: "user789",
      isUserPost: false
    },
  ];
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto p-4">
        <nav className="flex mb-6 border-b pb-4">
          ...
          <Link href="/dashboard/navigate" className="px-4 py-2 text-blue-600 font-medium border-b-2 border-blue-600">Navigate</Link>
        </nav>

        <h1 className="text-2xl font-bold text-gray-800 mb-6">Content Navigator</h1>
              ...
              {sources.map(source => (
                <div 
                  ...
                  className={`p-3 border-b border-gray-100 cursor-pointer ${
                    selectedSource === source.id ? 'bg-blue-50' : 'hover:bg-gray-50'
                  }`}
                >
                  <div className="flex items-center">
                    <div className="font-medium text-gray-700">{getSourceDomain(source.url)}</div>
                    {source.isUserSource && (
