...
export default function MagazinePage() {
  ...
  const fetchMagazineContent = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/magazine-content?days=${days}&skipCache=true&t=${timestamp}`);
      
      if (!response.ok) {
        throw new Error(`Failed to fetch magazine content: ${response.statusText}`);
      }
      ...
      if (data.featuredPosts && data.featuredPosts.length > 0) {
        console.log("Featured post summaries:", data.featuredPosts.map(p => p.aiSummary || 'No summary'));
      }
      ...
      console.log(`Found ${allSummaries.length} posts, ${allSummaries.filter(Boolean).length} with summaries`);
    ...
    const fetchSummaryFromAPI = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          if (data.summary) {
            console.log(`Received summary for post ${post.id}: ${data.summary}`);
            setLocalSummary(data.summary);
            
            // Store in local storage
            const storageKey = `summary_${post.id}_${post.title.substring(0, 20)}`;
      ...
      >
        ...
        <div className="border-l-4 border-blue-500 pl-4 mb-4 bg-blue-50 p-4 rounded-r flex-grow">
          <div className="flex flex-col">
            <div className="text-xs uppercase tracking-wider text-blue-700 mb-1 font-semibold">AI Summary</div>
        ...
        <div className="flex justify-between items-center text-sm text-gray-500 mt-auto pt-4 border-t border-gray-100">
          <span>By <a href={`/${post.username}/feed`} className="hover:underline">{post.username}</a></span>
    ...
    return (
      <div key={section.title || section.type} className="mb-12">
        <h2 className="text-3xl font-bold mb-6 pb-2 border-b">
          {section.title || (section.type === "source_spotlight" ? `From ${section.sourceName}` : "Recent Content")}
        </h2>
        ...
        <>
          <div className="mb-10 text-sm text-gray-500 bg-gray-50 p-4 rounded-lg">
            <div className="flex flex-wrap gap-x-6 gap-y-2 items-center justify-center">
              ...
              <span>•</span>
              <span>Generated {formatDistanceToNow(new Date(magazineContent.publishDate), { addSuffix: true })}</span>
            </div>
          </div>
          
          {/* Featured articles in masonry layout */}
          {magazineContent.featuredPosts && magazineContent.featuredPosts.length > 0 && (
            <div className="mb-12">
              <h2 className="text-3xl font-bold mb-6 pb-2 border-b">Today's Picks</h2>
          ...
          <div className="mb-12 space-y-16">
            ...
            <div>
              ...
              {magazineContent.editorialSections && magazineContent.editorialSections.length > 0 ? (
